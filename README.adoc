...
On Gentoo Linux you can simply use https://github.com/cvut/gentoo-overlay/tree/master/dev-lua/ngx-oauth[dev-lua/ngx-oauth] ebuild from the https://github.com/cvut/gentoo-overlay[CVUT Overlay].

. Add CVUT overlay, read instructions https://github.com/cvut/gentoo-overlay#usage[here].

. Allow to install “unstable” packages `dev-lua/lua-cjson`, `dev-lua/luaossl`, and `dev-lua/ngx-oauth` (all three are available in the overlay):

    echo -e 'dev-lua/lua-cjson\ndev-lua/luaossl\ndev-lua/ngx-oauth' >> /etc/portage/package.accept_keywords/ngx-oauth

. Install `dev-lua/ngx-oauth`:
